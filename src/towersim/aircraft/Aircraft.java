...
public abstract class Aircraft implements OccupancyLevel, Tickable, EmergencyState, Encodable {
    ...
    public boolean equals(Object obj) {
        if (this == obj) {
            // obj and this are same instance of Aircraft
            return true;
        } else if (!(obj instanceof Aircraft)) {
            // obj is not an instance of Aircraft
            return false;
        } else {
            // check whether two objects have same
            // callsign and Characteristics
            Aircraft aircraft = (Aircraft) obj;
            return this.getCallsign().equals(aircraft.getCallsign())
                    && this.getCharacteristics() == aircraft.getCharacteristics();
        }
    }
     ...
     */
    public int hashCode() {
        return this.getCharacteristics().hashCode() * 3 + this.getCallsign().hashCode() * 5;
    }
    ...
    public String encode() {
        ...
        result.add(this.getCallsign());
        result.add(this.getCharacteristics().name());
        result.add(this.getTaskList().encode());
        result.add(String.format("%.2f", this.getFuelAmount()));
        result.add("" + this.hasEmergency());
