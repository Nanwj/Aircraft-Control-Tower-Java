...
public class Task implements Encodable {
    ...
    public String encode() {
        // A machine-readable string of task
        String result = "" + this.getType();
        if (this.type == TaskType.LOAD) {
            result += "@" + this.getLoadPercent();
        }
        return result;
    }
    ...
    public boolean equals(Object obj) {
        if (this == obj) {
            // obj and this are same instance of Task
            return true;
        } else if (!(obj instanceof Task)) {
            // obj is not an instance of Task
            return false;
        } else {
            // check whether two objects have same
            // task type and load percent
            Task task = (Task) obj;
            return this.getType() == task.getType()
                    && this.getLoadPercent() == task.getLoadPercent();
        }
    }
    ...
    @Override
    public int hashCode() {
        return 11 + this.getType().hashCode() * 3 + this.getLoadPercent() * 5;
    }
}
